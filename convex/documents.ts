...
export const archive=mutation({
    ...
    handler:async(ctx,args)=>{
        ...
        const recursiveArchive=async(documenntId:Id<"documents">)=>{
            const children=await ctx.db.query("documents").withIndex("by_user_parent",(q)=>(
                q.eq("userId",userId)
                .eq("parentDocument",documenntId)
            ))
            .collect();
 ...
 export const getSidebar=query({
    ...
    handler:async(ctx,args)=>{
        ...
        .eq("userId",userId)
        .eq("parentDocument",args.parentDocument)).filter((q)=>
...
export const getTrash=query({
    handler:async(ctx)=>{
        ...
        if(!identity){
            throw new Error("Not authenticared");
        }
        const userId=identity.subject;
