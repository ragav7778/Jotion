...
export const Item=({
    id,
    label,
    onClick,icon:Icon,active,documentIcon,isSearch,level=0,onExpand,expanded
}:ItemProps)=>{
   ...
   event.stopPropagation;
    ...
    const onCreate=(event:React.MouseEvent<HTMLDivElement,MouseEvent>)=>{
        event.stopPropagation();
        if(!id) return;
        const promise=create({title:"Untitled",parentDocument:id}).then((document)=>{
            if(!expanded){
                onExpand?.();
            }
        ...
        className={cn("group min-h-[27px] text-sm py-1 pr-3 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium",active && "bg-primary/5 text-primary")}>
            {!!id&& (
                ...
                onClick={ handleExpand}>
                    <ChevronIcon
                    className="h-4 w-4 shronk-0 text-muted-foreground/50"/>
                </div>
            )}
            {
                documentIcon ?(
            ...
            {isSearch &&(
                ...
                gap-1 rounded border bg-mutedpx-1.5 font-mono opacity-100 ">
          ...
          {!!id && (
            <div className="ml-auto flex items-center gap-x-2">
                ...
                <div
                role="button"
                onClick={onCreate} className="opacity-0 group-hover:opacity-100 h-full
